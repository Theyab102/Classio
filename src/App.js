...
function ClassioSidebar({ screen, homeTab, onNavigate, character, onOpenCharacter, onToggleTheme, onOpenSearch, isMobile, user, isGuest, onSignOut, sidebarExpanded, onToggleSidebar, folderTab, onFolderTab, fileTab, onFileTab, folder, file }) {
  ...
  // Dashboard has no sidebar nav — star logo is the home button
  // Folder and document contexts provide their own nav
  const NAV = context === "dashboard" ? [] : context === "folder" ? [
    ...
    { id:"ai",      label:"AI Assistant", icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg> },
  ] : [
...
// ── Blank Document Modal ──────────────────────────────────────────────────────
function BlankDocModal({ onClose, onSave }) {
  const [name, setName] = useState("Untitled Document");
  const [body, setBody] = useState("");
  const T = useTheme();
  return (
    <Modal onClose={onClose}>
      <h2 style={{ fontFamily:"'Fraunces',serif", fontSize:22, fontWeight:700, color:T.text, marginBottom:16 }}>New Blank Document</h2>
      <input value={name} onChange={e=>setName(e.target.value)} placeholder="Document title…"
        style={{ width:"100%", border:`1.5px solid ${T.accentS}`, borderRadius:10, padding:"10px 14px",
          fontSize:15, fontWeight:600, outline:"none", color:T.text, background:T.bg, marginBottom:12, boxSizing:"border-box" }} />
      <textarea value={body} onChange={e=>setBody(e.target.value)} placeholder="Start writing here, or leave blank and write in the Notes tab after opening…"
        style={{ width:"100%", minHeight:180, border:`1.5px solid ${T.border}`, borderRadius:10, padding:"12px 14px",
          fontSize:14, lineHeight:1.8, outline:"none", resize:"vertical", color:T.text, background:T.bg,
          fontFamily:"'DM Sans',sans-serif", boxSizing:"border-box", marginBottom:16 }} />
      <div style={{ display:"flex", gap:8 }}>
        <button onClick={onClose} style={{ flex:1, padding:"10px", border:`1px solid ${T.border}`, borderRadius:10, background:"transparent", cursor:"pointer", fontSize:14, color:T.muted }}>Cancel</button>
        <button onClick={()=>onSave(name.trim()||"Untitled", body)}
          style={{ flex:2, padding:"10px", background:"linear-gradient(135deg,#7C5CFC,#3D8EF8)", color:"#fff", border:"none", borderRadius:10, cursor:"pointer", fontSize:14, fontWeight:700 }}>
          Create & Open
        </button>
      </div>
    </Modal>
...
export default function App() {
  ...
  const [showBlankDoc, setShowBlankDoc] = useState(false);
  ...
  return (
    <div style={{ minHeight:"100vh", background:T.bg, fontFamily:"'DM Sans',sans-serif", display:"flex", width:"100%", boxSizing:"border-box" }}>
      ...
      {/* ── Record Audio Modal ── */}
      {showBlankDoc && (
        <BlankDocModal onClose={()=>setShowBlankDoc(false)}
          onSave={(name, content) => {
            const inbox = folders.find(f=>f.id==="inbox") || {id:"inbox",name:"Inbox",color:"#6B4E8A",files:[]};
            const newDoc = { id:`fi${Date.now()}`, name:name||"Untitled Document", type:"text/plain", size:0,
              colorIndex:0, notes:content||"", studyCards:[], uploadedAt:new Date().toLocaleDateString(), linkedFileIds:[], isBlankDoc:true };
            setFoldersSave([{...inbox,files:[...inbox.files,newDoc]}, ...folders.filter(f=>f.id!=="inbox")]);
            setShowBlankDoc(false);
            setActiveFile({...newDoc, _fileObj:null});
            setActiveFolder({...inbox,files:[...inbox.files,newDoc]});
            setScreen("file"); setFileTab("notes");
          }} />
      )}
      {showRecordModal && (
        <AudioRecordModal
